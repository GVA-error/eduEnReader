/****************************************************************************
** Meta object code from reading C++ file 'asr.h'
**
** Created by: The Qt Meta Object Compiler version 67 (Qt 5.8.0)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "../../eduEnReader/ASR/asr.h"
#include <QtCore/qbytearray.h>
#include <QtCore/qmetatype.h>
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'asr.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 67
#error "This file was generated using the moc from 5.8.0. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
QT_WARNING_PUSH
QT_WARNING_DISABLE_DEPRECATED
struct qt_meta_stringdata_IASR_t {
    QByteArrayData data[5];
    char stringdata0[48];
};
#define QT_MOC_LITERAL(idx, ofs, len) \
    Q_STATIC_BYTE_ARRAY_DATA_HEADER_INITIALIZER_WITH_OFFSET(len, \
    qptrdiff(offsetof(qt_meta_stringdata_IASR_t, stringdata0) + ofs \
        - idx * sizeof(QByteArrayData)) \
    )
static const qt_meta_stringdata_IASR_t qt_meta_stringdata_IASR = {
    {
QT_MOC_LITERAL(0, 0, 4), // "IASR"
QT_MOC_LITERAL(1, 5, 14), // "recognizeBegan"
QT_MOC_LITERAL(2, 20, 0), // ""
QT_MOC_LITERAL(3, 21, 17), // "recognizeFinished"
QT_MOC_LITERAL(4, 39, 8) // "fileName"

    },
    "IASR\0recognizeBegan\0\0recognizeFinished\0"
    "fileName"
};
#undef QT_MOC_LITERAL

static const uint qt_meta_data_IASR[] = {

 // content:
       7,       // revision
       0,       // classname
       0,    0, // classinfo
       2,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       2,       // signalCount

 // signals: name, argc, parameters, tag, flags
       1,    0,   24,    2, 0x06 /* Public */,
       3,    1,   25,    2, 0x06 /* Public */,

 // signals: parameters
    QMetaType::Void,
    QMetaType::Void, QMetaType::QString,    4,

       0        // eod
};

void IASR::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        IASR *_t = static_cast<IASR *>(_o);
        Q_UNUSED(_t)
        switch (_id) {
        case 0: _t->recognizeBegan(); break;
        case 1: _t->recognizeFinished((*reinterpret_cast< const QString(*)>(_a[1]))); break;
        default: ;
        }
    } else if (_c == QMetaObject::IndexOfMethod) {
        int *result = reinterpret_cast<int *>(_a[0]);
        void **func = reinterpret_cast<void **>(_a[1]);
        {
            typedef void (IASR::*_t)();
            if (*reinterpret_cast<_t *>(func) == static_cast<_t>(&IASR::recognizeBegan)) {
                *result = 0;
                return;
            }
        }
        {
            typedef void (IASR::*_t)(const QString & );
            if (*reinterpret_cast<_t *>(func) == static_cast<_t>(&IASR::recognizeFinished)) {
                *result = 1;
                return;
            }
        }
    }
}

const QMetaObject IASR::staticMetaObject = {
    { &QObject::staticMetaObject, qt_meta_stringdata_IASR.data,
      qt_meta_data_IASR,  qt_static_metacall, Q_NULLPTR, Q_NULLPTR}
};


const QMetaObject *IASR::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->dynamicMetaObject() : &staticMetaObject;
}

void *IASR::qt_metacast(const char *_clname)
{
    if (!_clname) return Q_NULLPTR;
    if (!strcmp(_clname, qt_meta_stringdata_IASR.stringdata0))
        return static_cast<void*>(const_cast< IASR*>(this));
    return QObject::qt_metacast(_clname);
}

int IASR::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QObject::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 2)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 2;
    } else if (_c == QMetaObject::RegisterMethodArgumentMetaType) {
        if (_id < 2)
            *reinterpret_cast<int*>(_a[0]) = -1;
        _id -= 2;
    }
    return _id;
}

// SIGNAL 0
void IASR::recognizeBegan()
{
    QMetaObject::activate(this, &staticMetaObject, 0, Q_NULLPTR);
}

// SIGNAL 1
void IASR::recognizeFinished(const QString & _t1)
{
    void *_a[] = { Q_NULLPTR, const_cast<void*>(reinterpret_cast<const void*>(&_t1)) };
    QMetaObject::activate(this, &staticMetaObject, 1, _a);
}
struct qt_meta_stringdata_ASRLinuxKaldi_t {
    QByteArrayData data[5];
    char stringdata0[61];
};
#define QT_MOC_LITERAL(idx, ofs, len) \
    Q_STATIC_BYTE_ARRAY_DATA_HEADER_INITIALIZER_WITH_OFFSET(len, \
    qptrdiff(offsetof(qt_meta_stringdata_ASRLinuxKaldi_t, stringdata0) + ofs \
        - idx * sizeof(QByteArrayData)) \
    )
static const qt_meta_stringdata_ASRLinuxKaldi_t qt_meta_stringdata_ASRLinuxKaldi = {
    {
QT_MOC_LITERAL(0, 0, 13), // "ASRLinuxKaldi"
QT_MOC_LITERAL(1, 14, 16), // "stringRecognized"
QT_MOC_LITERAL(2, 31, 0), // ""
QT_MOC_LITERAL(3, 32, 19), // "allStringRecognized"
QT_MOC_LITERAL(4, 52, 8) // "exitCode"

    },
    "ASRLinuxKaldi\0stringRecognized\0\0"
    "allStringRecognized\0exitCode"
};
#undef QT_MOC_LITERAL

static const uint qt_meta_data_ASRLinuxKaldi[] = {

 // content:
       7,       // revision
       0,       // classname
       0,    0, // classinfo
       2,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       0,       // signalCount

 // slots: name, argc, parameters, tag, flags
       1,    0,   24,    2, 0x0a /* Public */,
       3,    1,   25,    2, 0x0a /* Public */,

 // slots: parameters
    QMetaType::Void,
    QMetaType::Void, QMetaType::Int,    4,

       0        // eod
};

void ASRLinuxKaldi::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        ASRLinuxKaldi *_t = static_cast<ASRLinuxKaldi *>(_o);
        Q_UNUSED(_t)
        switch (_id) {
        case 0: _t->stringRecognized(); break;
        case 1: _t->allStringRecognized((*reinterpret_cast< int(*)>(_a[1]))); break;
        default: ;
        }
    }
}

const QMetaObject ASRLinuxKaldi::staticMetaObject = {
    { &IASR::staticMetaObject, qt_meta_stringdata_ASRLinuxKaldi.data,
      qt_meta_data_ASRLinuxKaldi,  qt_static_metacall, Q_NULLPTR, Q_NULLPTR}
};


const QMetaObject *ASRLinuxKaldi::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->dynamicMetaObject() : &staticMetaObject;
}

void *ASRLinuxKaldi::qt_metacast(const char *_clname)
{
    if (!_clname) return Q_NULLPTR;
    if (!strcmp(_clname, qt_meta_stringdata_ASRLinuxKaldi.stringdata0))
        return static_cast<void*>(const_cast< ASRLinuxKaldi*>(this));
    return IASR::qt_metacast(_clname);
}

int ASRLinuxKaldi::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = IASR::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 2)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 2;
    } else if (_c == QMetaObject::RegisterMethodArgumentMetaType) {
        if (_id < 2)
            *reinterpret_cast<int*>(_a[0]) = -1;
        _id -= 2;
    }
    return _id;
}
QT_WARNING_POP
QT_END_MOC_NAMESPACE
