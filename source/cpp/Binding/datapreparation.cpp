...
void DataPreparation::writePreparation(WavWorker::Settings& settings, const QDir& outDirrectory)
{
    if (outDirrectory.exists() == false)
        QDir().mkdir(outDirrectory.absolutePath());
    // Щитаем шаг
...
void DataPreparation::splitFile(const QString& fileName, qreal minSplitSize, qreal maxSplitSize, const QDir& outDirrectory)
{
    ...
    WavWorker::Settings settings;
    writePreparation(settings);
    clearDir(SPLIT_DIR.absolutePath());
...
{
    QFileInfoList thrshList = QDir().entryInfoList(QStringList("*.wav"));
    for (auto trash : thrshList)
    {
        QString trashFIle = trash.absoluteFilePath();
        QFile::remove(trashFIle);
    }
}

bool DataPreparation::copy(const QDir& originalDir, const QDir& copyDir, const QStringList& mask)
{
    bool rez = true;
    QFileInfoList originalFiles = originalDir.entryInfoList(mask);
    if (originalFiles.size() == 0)
        return false;
    for (auto file : originalFiles)
    {
        QString base = file.fileName();
        QString copyName = copyDir.absolutePath() + "/" + base;
        if (QFile::copy(file.absoluteFilePath(), copyName) == false)
            rez = false;
    }
    return rez;
}
